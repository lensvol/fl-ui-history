...
type SocialActActions = AddNewContactAction
  | ChooseBranchSuccessAction
  | FetchIneligibleContactsAction
  | SendSocialInviteAction;

export default function reducer(state: ISocialActState = INITIAL_STATE, action: SocialActActions): ISocialActState {
  switch (action.type) {
    ...
    case FETCH_INELIGIBLE_CONTACTS_SUCCESS:
      return {
        ...state,
        ineligibleContacts: (action.payload as { ineligibleContacts: IneligibleContact[] }).ineligibleContacts,
      };
