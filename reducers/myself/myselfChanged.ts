...
export default function myselfChanged(state: IMyselfState, action: { payload: { possession: IQuality }[] }) {
  const {
    categories,
    qualities,
  } = state;
  ...
  payload.forEach(({ possession: updatedQuality }) => {
    ...
    const category = newCategories.find(c => c.categories.includes(updatedQuality.category));
    if (!category) {
