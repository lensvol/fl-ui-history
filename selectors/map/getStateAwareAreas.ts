...
const getAreas: (state: IAppState) => IArea[] = (state: IAppState) => state.map.areas || [];
const getSetting = (state: IAppState) => state.map.setting! as IMappableSetting;
const getCurrentArea: (state: IAppState) => IArea | undefined = (state) => state.map.currentArea;

function outputFunc(areas: IArea[], setting: IMappableSetting, currentArea: IArea | undefined) {
