...
export function hasThingNaturedQualities(category: ICategory, allCategories: ICategoriesState) {
...
export function isNotAnEquippableCategory(category: ICategory): boolean {
  return (OUTFIT_CATEGORIES as string[])
    .indexOf(category.name.replace(/ /g, '')) < 0;
}
