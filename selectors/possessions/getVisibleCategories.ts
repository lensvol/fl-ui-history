...
export function matchesFilterString({
  filterString,
}: {
  filterString: string;
}): (q: IQuality | undefined) => boolean {
  return (q: IQuality | undefined) =>
    q !== undefined && normalize(q.name).indexOf(normalize(filterString)) >= 0;
}
