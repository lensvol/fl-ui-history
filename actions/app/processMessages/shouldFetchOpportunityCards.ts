...
type QualityEffectWithPossession = ApiResultMessageQualityEffect & {
  possession: IQuality;
};
...
export default function shouldFetchOpportunityCards(
  ...
  const qualityRelevantToCurrentPlansHasChanged = defaultMessages
    ...
    .some(
      ({ possession: { id } }) =>
        id && opportunityQualityRequirementIds.indexOf(id) >= 0
    );

  const areaHasChanged = defaultMessages.some(
    (m) => m.type === AREA_CHANGE_MESSAGE
  );
  const settingHasChanged = defaultMessages.some(
    ({ type }) => type === SETTING_CHANGE_MESSAGE
  );

  return (
    hasDeckBeenRefreshed ||
    qualityRelevantToCurrentPlansHasChanged ||
    settingHasChanged ||
    areaHasChanged
  );
}
