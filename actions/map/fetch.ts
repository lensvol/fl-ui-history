...
export type FetchMapAction = FetchMapRequested | FetchMapSuccess | FetchMapFailure;

const fetchMapRequested: ActionCreator<FetchMapRequested> = () => ({
  type: MapActionTypes.FETCH_MAP_REQUESTED,
});

const fetchMapSuccess: ActionCreator<FetchMapSuccess> = (data: IFetchMapResponse) => ({
...
export function fetch(service: IMapService) {
  return (options?: IBootstrapOptions) => async (dispatch: ThunkDispatch<any, any, any>, getState: () => IAppState) => {
    ...
    try {
      ...
      if (data.areas) {
        ...
        const { map: { fallbackMapPreferred } } = getState();
        ...
        if (fallbackMapPreferred || !isWebGLSupported()) {
          ...
          if ((fetchSpritesNow ?? true) && setting?.mapRootArea?.areaKey) {
            ...
            await loadAndDrawMapSprites(
              data.areas.filter((area: IArea) => isDrawable(area) && !isLodgings(area)),
              setting as IMappableSetting,
              options?.onSpriteLoadProgress,
            );
            // Create state-aware areas, then update their sprites
            const stateAwareAreas: IStateAwareArea[] = data.areas
              .map((area: IArea) => asStateAwareArea(
                area,
                data.areas,
                setting as IMappableSetting,
                data.currentArea,
              ));
            await Promise.all(stateAwareAreas.map(SpriteCaches.updateSpriteForArea));
          }
        }
      }
