...
function FilterByEnhancementDropDown({
  ...
  const onChange = useCallback(({ qualityId }) => {
  ...
  const options = useMemo(
    () => [
      OPTION_SHOW_ALL,
      ...enhancements.map(e => ({ ...e, qualityName: e.qualityName.replace(/:$/, '') })),
    ].map(e => ({
      label: e.qualityName,
      value: e.qualityId,
  ...
  const filteredOptionsWithSeparators = useMemo(() => options
    ...
    .map((e, idx, src) => {
      ...
      const needsSeparator = idx > 0
        ...
        && (idx > 1 || selectedEnhancementQualityId !== QUALITY_ID_DUMMY_SHOW_ALL_ITEMS);
...
export default withFeature(FILTER_ENHANCEMENTS)(connect(mapStateToProps)(FilterByEnhancementDropDown));
