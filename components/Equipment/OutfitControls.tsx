...
export function OutfitControls({
  ...
  const [isPurchaseOutfitSlotModalOpen, setIsPurchaseOutfitSlotModalOpen] = useState(false);
  const [isOutfitChangeErrorModalOpen, setIsOutfitChangeErrorModalOpen] = useState(false);
  const [outfitChangeErrorMessage, setOutfitChangeErrorMessage] = useState<string | undefined>(undefined);

  const selectedOutfit: IOutfit | undefined = useMemo(() => outfits.find(o => o.selected), [outfits]);

  const onSelectOutfit = useCallback(async (id: number | 'buy-new-outfit') => {
    if (isChanging) {
      return;
    }
      ...
      <Feature name={FEATURE_POSSESSIONS_TAB_AVATAR}>
        {(shouldShowAvatar: boolean) => (
          ...
          >
            ...
            <div
              className="outfit-controls__dropdown-container"
            >
              <div
                className="outfit-controls__dropdown-and-buttons"
              >
                <div
                  style={{
                    alignItems: 'center',
...
function OutfitChangeErrorModal(props: ReactModalProps & { message: string | undefined }) {
  const {
    isOpen,
    message,
    onAfterClose,
...
type Props = ReturnType<typeof mapStateToProps> & OwnProps & { dispatch: ThunkDispatch<any, any, any> };
