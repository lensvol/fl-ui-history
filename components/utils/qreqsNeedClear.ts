export default function qreqsNeedClear(el: Element) {
  ...
  const cardWidth = 130;
  const buttons: ChildNode[] = Array.from(
    el.querySelector(".buttons")?.childNodes ?? []
  );
  ...
  const buttonsWidth = [...buttons].reduce((acc, childNode) => {
    ...
    const elementWidth =
      el2.clientWidth +
      pixelify(el2 as Element, "margin-left") +
      pixelify(el2, "margin-right");
