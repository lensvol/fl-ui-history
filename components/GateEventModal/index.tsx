...
export class GateEventModal extends React.Component<Props, State> {
  ...
  fetchAvailable = async () => {
    ...
    try {
      ...
      const { data }: { data: IApiStoryletResponseData } =
        await new StoryletService().fetchAvailable();
  ...
  handleChooseBranch = async (
    ...
    }: IApiStoryletResponseData = await dispatch(
      chooseGateEventBranch(branchData)
    );
    ...
    ].find(({ type }: IHasTypeString) => type === SETTING_CHANGE_MESSAGE);
    ...
    return (
      <ReactModal
        ...
        style={{
          ...
          content: {
            backgroundImage:
              "https://images.fallenlondon.com/static/bg-paper-dark.png",
