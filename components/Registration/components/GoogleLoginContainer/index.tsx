...
export function GoogleLoginContainer({
  history,
  label,
}: Props) {
  ...
  const [areThirdPartyCookiesAvailable, setAreThirdPartyCookiesAvailable] = useState(true);
...
function GoogleLoginErrorModal(props: { isOpen: boolean, onRequestClose: () => void, errors: GoogleError[] }) {
  const {
    errors,
    isOpen,
    onRequestClose,
  ...
  return (
    ...
    >
      <div>
        ...
        <p>
          Something went wrong while logging in with Google. Please get in touch with us at
          {' '}
          <a
          ...
          {' '}
          and let us know about the following errors, which are what Google can tell us
        ...
        >
          {errors.map((e, i) => ({ ...e, index: i })).map(({ error, details, index }) => (
            <li key={index}>
              <p>
                <span style={{ fontFamily: 'monospace' }}>{error}</span>
                :
                {' '}
                {details ? <span>{details}</span> : <span style={{ fontStyle: 'italic' }}>[no further details]</span>}
              </p>
            </li>
          ))}
        </ul>
