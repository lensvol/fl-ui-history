...
function PlanButtonlet({
  ...
  const onClickToAdd = useCallback(async () => {
    ...
    if (playerHasMaximumActivePlans) {
      alert(
        "Unfortunately, you already have the maximum number of active plans."
  ...
  }, [branch, dispatch, isMounted, playerHasMaximumActivePlans]);
  ...
  const isBranchPlanned = useMemo(
    () => !!activePlans.find((plan) => plan.branch.id === branch.id),
    [activePlans, branch.id]
  );
...
const mapStateToProps = ({ plans: { activePlans } }: IAppState) => ({
  activePlans,
});
