...
export function useCursor(area: IStateAwareArea, setting: ISetting | undefined, zoomLevel: number) {
  return useMemo(() => {
    ...
    if (!area.isLandmark && isInteractableAtThisZoomLevel(area, setting, zoomLevel) ? 'pointer' : 'grab') {
      return 'pointer';
    }
    return 'grab';
