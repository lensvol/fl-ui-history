...
export function UnterzeeFallbackMap(props: OwnProps & StateProps) {
  ...
  const labelledAreas = useMemo(() => areas.map(a => a as ILabelledStateAwareArea), [areas]);
  const areasWithSprites = useMemo(() => areas.filter(a => a.isDrawable).map(a => a as IHasSprite), [areas]);
  ...
  const { height: mapHeight, width: mapWidth } = getMapDimensionsForSetting({ mapRootArea: setting.mapRootArea });
  ...
  if (mappableSetting === undefined || minZoom === undefined || maxZoom === undefined) {
    return null;
  }
