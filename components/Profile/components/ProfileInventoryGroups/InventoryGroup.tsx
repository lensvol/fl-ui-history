...
export function InventoryGroup(props: Props) {
  const { data, hideHeadings, profile } = props;

  const inventoryItems = [...data.possessions].map((item) => (
    <InventoryItem
      key={item.id}
      data={item}
  ...
  return (
    <div
      className={classnames(
        "inventory-group",
        profile && "inventory-group--profile"
      )}
      data-group-name={data.name}
    >
      {!hideHeadings && (
        <h2 className="heading heading--2 inventory-group__heading">
          {data.name}
        </h2>
      )}
      <ul
