...
export default function App() {
  ...
  return (
    <ErrorBoundary>
      ...
      <Timer />
      <Router
        basename={Config.basePath || null}
        history={history}
      >
        <div className="router-example">
          <div className="ie11banner">
            Fallen London no longer supports Internet Explorer (support ended on 11 January 2020).
            {' '}
          ...
          <Route render={({ location }) => (
            <Switch location={location}>
              ...
              <Route path="/profile/:profileName/:fromEchoId?" exact component={ProfilePage} />
              ...
              <RequireUnauthenticated path="/login" exact component={LoginContainer} />

              {/* Routes that only logged-in users with no character can visit */}
              <RequireNoCharacter path="/create-character" exact component={CreateCharacter} />

              {/* Routes that only logged-in users with a character can visit */}
              <RequireCharacter path="/fate" exact component={FateTab} />
              <RequireCharacter path="/bazaar" exact component={ExchangeTab} />
              <RequireCharacter path="/possessions" exact component={PossessionsTab} />
              <RequireCharacter path="/myself" exact component={MyselfTab} />
              <RequireCharacter path="/messages" exact component={MessagesTab} />
