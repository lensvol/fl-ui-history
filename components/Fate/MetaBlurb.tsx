...
export default function MetaBlurb({ card }: { card: IFateCard | undefined }) {
  ...
  return (
    <div className="metablurb">
      ...
      />
      {factions && (
        <ul className="metablurb__faction-list" style={{ fontStyle: "italic" }}>
          {factions
            .split(",")
            .map((_) => _.trim())
            .map((faction) => (
              <li className="metablurb__faction-list-item" key={faction}>
                <span className="metablurb__faction-name">{faction}</span>
              </li>
            ))}
        </ul>
      )}
