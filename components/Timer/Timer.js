...
class Timer extends Component {
  ...
  onTick = () => {
    ...
    // If we aren't logged in, with a character and a user ID, or if the client is out of date, then return
    if (!this.shouldRenewCharacterData()) {
      return;
    }
  ...
  renewActions = () => {
    ...
    // If we're in the middle of fetching the sidebar, or if our actions are full, don't initiate a new fetch
  ...
  renewCards = () => {
    ...
    // If we're in the middle of fetching cards, don't initiate a new fetch
  ...
  // We're not rendering anything for this component, just keeping it running in the background
  render() {
    const {
      actions: { error },
    } = this.props;
...
const mapStateToProps = ({ actions, cards, timer, user, versionSync }) => ({
  actions,
  cards,
