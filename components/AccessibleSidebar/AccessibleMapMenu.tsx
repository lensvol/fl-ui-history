...
export function AccessibleMapMenu({
  ...
  const handleClick = useCallback(async (area: IArea) => {
    const {
      gateEvent,
      unlocked,
      id: areaId,
  ...
  if (!setting?.canOpenMap) {
    return (
      <div>
        Travel is currently unavailable.
      </div>
    );
  }
  ...
  return (
    <ul className="accessible-map-menu">
      {(areas || [])
        ...
        .map(area => <PossibleMapMenuItem key={area.areaKey} area={area} onClick={handleClick} />)}
    </ul>
  );
}
