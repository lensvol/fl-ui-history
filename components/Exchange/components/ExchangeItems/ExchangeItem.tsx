...
export default function ExchangeItem({
  ...
  const { purchaseQuality, quality } = availability;
  const purchaseIsCurrency = purchaseQuality.id === QUALITY_ID_PENNY;

  const onClick = useCallback(
    () => onStartTransaction(data),
    [data, onStartTransaction]
  );
  ...
  const altText = useMemo(
    ...
    [quality.description, quality.enhancements, quality.name]
  );
  ...
  return (
    ...
    >
      ...
      <div className="item__desc">
        <span className="js-item-name item__name">{quality.name}</span>
        <QualityValue
