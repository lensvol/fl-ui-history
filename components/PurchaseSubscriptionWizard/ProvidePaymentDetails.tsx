...
interface Props {
  ...
  on3dSecureSuccess: (purchaseRequest: ICreateBraintreeSubscriptionRequest) => void,
  onGoBack: () => void,
}
...
export default function ProvidePaymentDetails({
  ...
  const [reCaptchaValue, setReCaptchaValue] = useState<any | undefined>(undefined);
  ...
  const loading = useMemo(() => !instance || submitting, [instance, submitting]);

  const handleReCaptchaChange = useCallback((value) => {
    setReCaptchaValue(value);
  }, []);

  const handleClick = useCallback(async (_evt: React.MouseEvent<HTMLButtonElement>) => {
  ...
  return (
    <div>
      <Header
        amountString={braintreePlan && `${braintreePlan.currencyIsoCode} ${braintreePlan.price}`}
      />
      <BraintreeDropIn
      ...
      >
        <SubmitButton
          isReCaptchaComplete={!requireReCaptcha || (reCaptchaValue !== undefined)}
          loading={loading}
          onClick={handleClick}
        />
        <button
