...
export function PurchaseOutfitSuccess({
  ...
  const onSubmit = useCallback(
    async ({ name }: { name: string }) => {
      ...
      if (result instanceof Success) {
        const { data } = result;
        const newOutfit = data.character.outfits.find(
          (o: IOutfit) => o.name === name
        );
        if (newOutfit) {
    ...
    [dispatch, initialName, onFinishedRenaming, outfits]
  );
  ...
  return (
    <>
      ...
      <p>
        You spent {fateCard.price} Fate... And thank you! It's folk like you who
        keep the game running.
      </p>
      <p>Choose a name for your new outfit:</p>
      <Formik
        onSubmit={onSubmit}
