...
export default function getPositionForSprite(area: IArea, whatKind: SpriteType) {
  const {
    areaKey,
  ...
  switch (whatKind) {
    case 'available': {
      if (spriteTopLeftX === undefined || spriteTopLeftY === undefined) {
        console.error(`getPositionForSprite(${areaKey}): this area has no sprite coords`);
        console.error(area);
    ...
    case 'main-destination': {
      if (mainDestinationSpriteTopLeftX === undefined || mainDestinationSpriteTopLeftY === undefined) {
        console.error(`getPositionForSprite(${areaKey}): no main destination sprite coordinates`);
        return pointFromXY(DUMMY_XY_COORDINATES);
      }
      return new PIXI.Point(mainDestinationSpriteTopLeftX / 2, -mainDestinationSpriteTopLeftY / 2);
    }
    default:
      console.error(`I don't know what to do with this sprite: ${areaKey}-${whatKind}`);
      return pointFromXY(DUMMY_XY_COORDINATES);
  }
}
