...
export default function getSortedSpriteRecords(
  unsortedSpriteRecords: SpriteRecord[],
  areas: IArea[]
) {
  // Sort sprite records, putting districts lower in the stack
  const spriteRecords = [...unsortedSpriteRecords].sort(
    makeSpriteRecordComparator(areas)
  );

  // We need to load and render non-selections first, so that selections sit on top
  const notSelections = spriteRecords.filter(
    (sr) => SELECTION_SPRITE_SUFFIXES.indexOf(sr[1]) < 0
  );
  const selections = spriteRecords.filter(
    (sr) => SELECTION_SPRITE_SUFFIXES.indexOf(sr[1]) >= 0
  );
