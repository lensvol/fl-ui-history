...
export default function getFallbackMapImageURL(setting: IMappableSetting) {
  return `${MAP_BASE_URL}/fallback/fallback-${getFallbackImageNameForSetting(
    setting
  )}.jpg`;
}

function getFallbackImageNameForSetting(
  ...
  throw new Error(
    `I can't find the fallback image name for the Setting "${setting.name}"`
  );
}
