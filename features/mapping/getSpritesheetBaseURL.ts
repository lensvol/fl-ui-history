...
export default function getSpritesheetBaseURL({ mapRootArea }: Pick<IMappableSetting, 'mapRootArea'>) {
  const subfolder = getSubfolder({ mapRootArea });
  return `${MAP_BASE_URL}/spritesheets/${SPRITESHEET_VERSION}/${subfolder}`;
}

function getSubfolder({ mapRootArea: { areaKey } }: Pick<IMappableSetting, 'mapRootArea'>) {
