...
export const findParentArea = (area: IArea, areas: IArea[]) =>
  areas.find(
    (parent) =>
      !!(parent.childAreas || []).find(
        (child) => child.areaKey === area.areaKey
      )
  );

export function getHitboxForArea(
  area: IArea,
...
export const isCurrentArea = (area: IArea, currentArea: IArea | undefined) =>
  area.areaKey === currentArea?.areaKey;
...
export const isInteractable = (a: IArea) =>
  a.canMoveTo && (a.unlocked || !!a.gateEvent);

export const isLandmark = (a?: IArea) => (a?.type ?? null) === "Landmark";

export const isLabelled = (a: IArea) =>
  (a.labelX ?? -1) >= 0 && (a.labelY ?? 1) <= 0;
...
export const isMappable = (area: IArea, areas: IArea[]) => {
  ...
  return !(
    ...
    (area.labelY ?? Number.MAX_SAFE_INTEGER) > 0
  );
};

export const isSelectable = (a?: IArea) =>
  (a?.unlocked ?? false) || (a?.discovered ?? false);

// TODO(sdob): This needs to return false when a child area of the district has been discovered
export const isUndiscoveredDistrict = (a: IArea) =>
  isDistrict(a) && !a.discovered;

export const isUnlockable: (_: IArea) => boolean = (a: IArea) => !!a.gateEvent;

export const isUnterzeePlanningSetting = (s: ISetting | undefined) =>
  s?.id === SETTING_ID_ABOARD_AT_PORT;
...
export const shouldShowGateIcon = (area: IArea) =>
  !area.unlocked && isUnlockable(area);
...
export const shouldShowBorderFanciness = (
  area: IArea,
  setting: ISetting | undefined,
  zoomLevel: number
) => {
  ...
  return (
    zoomLevel >= getMinimumZoomLevelForDestinations(setting as IMappableSetting)
  );
};

export const shouldShowMainDestination = (area: IArea) =>
  isDistrict(area) && area.unlocked;

export const shouldZoomOnTapAtZoomLevel = (
  area: IArea,
  setting: ISetting | undefined,
  zoomLevel: number
) => {
...
export const slugify = (name: string) =>
  baseSlugify(name.replace(/^[Tt]he /, "").replace(/'/, ""), {});
